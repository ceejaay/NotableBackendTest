...
const {dateTimeParse, middleware2} = require('./middleware.js')
...
server.delete("/appointments/:id", (req, res) => {
  const {id} = req.params
    db('appointments')
      ...
      .catch(error => res.status(500).json({message: "could not delete user", error}))
})
...
server.post("/doctors/:id/appointments", timeParse, (req, res) => {
  ...
  db('appointments')
    .insert({patient_first_name: patient_first_name, patient_last_name: patient_last_name, time: time, date: date, new_patient: new_patient, doctor_id: id})
    .then(id => res.status(201).json(id))
    .catch(error => res.status(500).json({message: "There was a problem creating the resource", error}))
})
